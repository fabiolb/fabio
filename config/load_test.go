...
func TestLoad(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			args: []string{"-proxy.addr", ":5555;cs=name;strictmatch=true;proto=prom-https", "-proxy.cs", "cs=name;type=path;cert=foo;clientca=bar;refresh=2s;hdr=a: b;caupgcn=furb"},
			cfg: func(cfg *Config) *Config {
				cfg.Listen = []Listen{
					{
						Addr:        ":5555",
		...
		},
		{
			...
			cfg:  func(cfg *Config) *Config { return nil },
			err:  errors.New("cert source requires proto 'https', 'tcp', 'tcp-dynamic', 'https+tcp+sni', 'prom-http', 'prom-https' or 'grpcs'"),
		},
		{
