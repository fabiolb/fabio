...
func load(p *properties.Properties) (cfg *Config, err error) {
	...
	f.BoolVar(&cfg.Proxy.StripPath, "proxy.stripPath", Default.Proxy.StripPath, "path stripping while forwarding requests")
	...
	f.BoolVar(&cfg.Registry.Consul.ByServiceName, "registry.consul.register.byServiceName", Default.Registry.Consul.ByServiceName, "register services from consul by their service name")
