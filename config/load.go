...
func load(p *properties.Properties) (cfg *Config, err error) {
	...
	f.BoolVar(&cfg.Proxy.Log.Enable, "proxy.log.enable", Default.Proxy.Log.Enable, "enable written access log")
	f.StringVar(&cfg.Proxy.Log.Format, "proxy.log.format", Default.Proxy.Log.Format, "directive to change the format of logged")
	f.StringVar(&cfg.Proxy.Log.File.Path, "proxy.log.file.path", Default.Proxy.Log.File.Path, "directive to specify the location of the log ")
