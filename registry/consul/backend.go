...
func (b *be) ManualPaths() ([]string, error) {
	keys, _, err := listKeys(b.c, b.cfg.KVPath, 0, b.cfg.RequireConsistent)
	return keys, err
}
...
func (b *be) ReadManual(path string) (value string, version uint64, err error) {
	...
	return getKV(b.c, b.cfg.KVPath+path, 0, b.cfg.RequireConsistent)
}
...
func (b *be) WatchManual() chan string {
	...
	kv := make(chan string)
	go watchKV(b.c, b.cfg.KVPath, kv, true, b.cfg.RequireConsistent)
	return kv
}
...
func (b *be) WatchNoRouteHTML() chan string {
	...
	go watchKV(b.c, b.cfg.NoRouteHTMLPath, html, false, b.cfg.RequireConsistent)
	return html
}
