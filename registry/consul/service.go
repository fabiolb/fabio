...
// on every change.
func watchServices(client *api.Client, tagPrefix string, acceptableStatus []string, config chan string) {
	...
	for {
		...
		config <- servicesConfig(client, passingServices(checks, acceptableStatus), tagPrefix)
