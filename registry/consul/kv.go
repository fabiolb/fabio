...
func watchKV(client *api.Client, path string, config chan string, separator bool, requireConsistent bool) {
	var lastIndex uint64
	var lastValue string

	for {
		value, index, err := listKV(client, path, lastIndex, separator, requireConsistent)
		if err != nil {
...
func listKeys(client *api.Client, path string, waitIndex uint64, requireConsistent bool) ([]string, uint64, error) {
	q := &api.QueryOptions{RequireConsistent: requireConsistent, WaitIndex: waitIndex}
...
func listKV(client *api.Client, path string, waitIndex uint64, separator bool, requireConsistent bool) (string, uint64, error) {
	q := &api.QueryOptions{RequireConsistent: requireConsistent, WaitIndex: waitIndex}
	kvpairs, meta, err := client.KV().List(path, q)
	if err != nil {
		return "", 0, err
...
func getKV(client *api.Client, key string, waitIndex uint64, requireConsistent bool) (string, uint64, error) {
	q := &api.QueryOptions{RequireConsistent: requireConsistent, WaitIndex: waitIndex}
